...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uMRC
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1440000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1440000);
...
void ThreadScriptCheck() {
    RenameThread("marketcoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1389497608, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xb2a40d74 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xb2, 0xa4, 0x0d, 0x74 };
